...
## 🧪 Testing Checklist for QA & Testers

### 1. Soft Deletion & 30-Day Trash Retention
- [ ] Delete a Client, Team Member, or Audit Record.
- [ ] Verify that the item is **NOT** permanently removed from the database immediately.
- [ ] Verify the item appears at the bottom in the **"Recently Deleted (30-Day Trash)"** section.
- [ ] Test **Restore**: Click **Restore** on a soft-deleted item and verify it returns to active status.
- [ ] Test **Permanent Purge**: As an Admin, click **Purge** to permanently delete an item.

### 2. Visual Dimming & Status Filter Division
- [ ] Observe soft-deleted items in lists: verify they render with **55% Opacity** and a prominent **`DELETED (X days left)`** red badge.
- [ ] Test the **Status Filter Dropdown**:
  - Select **"Active Only"**: Only active records are displayed.
  - Select **"Deleted / Trash"**: Only soft-deleted records within 30 days are displayed.
  - Select **"All Statuses"**: Both active and soft-deleted records are shown (with deleted items dimmed).

### 3. Password Visibility Eye Toggle Icon
- [ ] Go to any password field across the app (Login, Change Password, User Edit popup).
- [ ] Click the **Eye Icon** on the right of the textbox to toggle visibility of hidden password text.

### 4. Navigation Interception & Unsaved Draft Modal
- [ ] Start editing or creating an Audit/Tax/Secretarial form.
- [ ] Attempt to switch section tabs or navigate away without saving.
- [ ] Verify the **"Sure want to save as draft?"** modal appears with **Save as Draft** and **Discard** options.

### 5. Dashboard Branch Activity Card
- [ ] Check the **Dashboard Activity Card**.
