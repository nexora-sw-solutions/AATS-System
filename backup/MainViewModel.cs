...
public record NavigationState(ViewModelBase ViewModel, string Main, string Sub, string Leaf, string ActivePage);
...
public partial class MainViewModel : ViewModelBase
{
    ...
    private void InitializeSearchItems()
    {
        _allModules = new List<ModuleSearchItem>
        {
            ...
            new() { Name = "Form - 15", Route = "/secretarial/form-15", Category = "Secretarial & Advisory", Command = NavigateToForm15Command },
    ...
    private void NavigateTo(ViewModelBase viewModel, string main, string sub = "", string leaf = "", string activePage = "")
    {
        ...
        {
            _backStack.Push(new NavigationState(CurrentView, BreadcrumbMain, BreadcrumbSub, BreadcrumbLeaf, ActivePage));
            _forwardStack.Clear();
            UpdateCommandStates();
        }
    ...
    private void GoBack()
    {
        ...
        _forwardStack.Push(new NavigationState(CurrentView!, BreadcrumbMain, BreadcrumbSub, BreadcrumbLeaf, ActivePage));
        
        var state = _backStack.Pop();
        NavigateTo(state.ViewModel, state.Main, state.Sub, state.Leaf, state.ActivePage);
    ...
    private void NavigateToForm15Detail(AuditRecord record)
    {
        ...
        vm.NavigateToEditRecord = (r) => ExecuteAuthorizedAction(() =>
        {
            var addVm = new AddForm15ViewModel(r);
            addVm.GoBack = async () => 
            {
                // To refresh the detail view after edit, we can reload or just go back to table
                // NavigateToForm15 will reload the list entirely. Let's return to table.
                // Or wait, standard pattern is to reload detail or go back. Let's go back to Form15 list.
                NavigateToForm15();
            };
            NavigateTo(addVm, "Secretarial & Advisory", "Form - 15", "Edit Record", "Form15");
        });
    ...
    {
        if (CurrentUser?.IsStaff == true && !OtpService.Instance.IsAuthorized)
        {
