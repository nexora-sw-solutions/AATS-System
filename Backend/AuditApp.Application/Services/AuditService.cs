...
public class AuditService : IAuditService
{
    ...
    public async Task<AuditRecordResponse> CreateRecordAsync<TEntity>(CreateAuditRecordRequest request, CancellationToken ct = default) where TEntity : AuditBaseEntity
    {
        ...
        record.Process = request.Process ?? request.Status ?? "Pending"; 
        ...
        record.TotalPayment = request.TotalPayment != 0 ? request.TotalPayment : request.TotalFee;
        record.PartialAmount = request.PartialAmount != 0 ? request.PartialAmount : request.PaidAmount;
        record.SubTotal = request.SubTotal != 0 ? request.SubTotal : request.ServiceFee;
        ...
        {
            assignmentProp.SetValue(record, request.Assignment ?? request.Description);
        }
    ...
    public async Task<AuditRecordResponse> UpdateRecordAsync<TEntity>(Guid id, UpdateAuditRecordRequest request, CancellationToken ct = default) where TEntity : AuditBaseEntity
    {
        ...
        if (request.SubTotal.HasValue) record.SubTotal = request.SubTotal.Value;
        if (request.Discount.HasValue) record.Discount = request.Discount.Value;
        if (request.TotalFee.HasValue) record.TotalPayment = request.TotalFee.Value;
        if (request.TotalPayment.HasValue) record.TotalPayment = request.TotalPayment.Value;
        if (request.PaidAmount.HasValue) record.PartialAmount = request.PaidAmount.Value;
        if (request.PartialAmount.HasValue) record.PartialAmount = request.PartialAmount.Value;

        if (record is TaxAccountRecord tar && request.AssignedToId.HasValue)
        {
            tar.AssignedTo = request.AssignedToId.Value;
        }

        var assignmentProp = typeof(TEntity).GetProperty("Assignment");
        if (assignmentProp != null)
        {
            if (request.Assignment != null) assignmentProp.SetValue(record, request.Assignment);
            else if (request.Description != null) assignmentProp.SetValue(record, request.Description);
        }
    ...
    private static AuditRecordResponse MapToResponse<TEntity>(TEntity r) where TEntity : AuditBaseEntity
    {
        return new AuditRecordResponse
        {
            ...
            Assignment = (r as dynamic).Assignment,
            Status = r.Process,
