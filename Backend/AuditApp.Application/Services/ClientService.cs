...
public class ClientService : IClientService
{
    ...
    {
        var query = _db.Clients.Include(c => c.Branch).AsQueryable();
    ...
    {
        var client = await _db.Clients.Include(c => c.Branch).FirstOrDefaultAsync(c => c.Id == id, ct);
    ...
    public async Task<ClientResponse> CreateAsync(CreateClientRequest request, CancellationToken ct = default)
    {
        ...
        {
            ClientCode = Guid.NewGuid().ToString("N")[..8].ToUpper(),
