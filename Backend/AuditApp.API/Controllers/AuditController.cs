...
public class AuditController : ControllerBase
{
    ...
    [HttpGet("tax")]
    public async Task<IActionResult> GetTax([FromQuery] PaginationParams p)
        => Ok(ApiResponse<PaginatedResult<AuditRecordResponse>>.Ok(await _auditService.GetRecordsAsync<Domain.Entities.TaxAccountRecord>(p)));

    [HttpGet("tax/{id}")]
    public async Task<IActionResult> GetTaxById(Guid id)
        => OkOrNotFound(await _auditService.GetRecordByIdAsync<Domain.Entities.TaxAccountRecord>(id));

    [HttpPost("tax")]
    public async Task<IActionResult> CreateTax(CreateAuditRecordRequest req)
        => Ok(ApiResponse<AuditRecordResponse>.Ok(await _auditService.CreateRecordAsync<Domain.Entities.TaxAccountRecord>(req)));

    [HttpPut("tax/{id}")]
    public async Task<IActionResult> UpdateTax(Guid id, UpdateAuditRecordRequest req)
        => Ok(ApiResponse<AuditRecordResponse>.Ok(await _auditService.UpdateRecordAsync<Domain.Entities.TaxAccountRecord>(id, req)));

    [HttpDelete("tax/{id}")]
    public async Task<IActionResult> DeleteTax(Guid id)
    { await _auditService.DeleteRecordAsync<Domain.Entities.TaxAccountRecord>(id); return Ok(ApiResponse<object>.Ok(null)); }

    [HttpPatch("tax/{id}/process")]
    public async Task<IActionResult> UpdateTaxProcess(Guid id, [FromBody] UpdateProcessRequest req)
        => Ok(ApiResponse<AuditRecordResponse>.Ok(await _auditService.UpdateProcessAsync<Domain.Entities.TaxAccountRecord>(id, req.Process)));

    [HttpPatch("tax/{id}/payment")]
    public async Task<IActionResult> UpdateTaxPayment(Guid id, [FromBody] UpdatePaymentDetailsRequest req)
        => Ok(ApiResponse<AuditRecordResponse>.Ok(await _auditService.UpdatePaymentAsync<Domain.Entities.TaxAccountRecord>(id, req.PaymentStatus, req.PaymentOption, req.SubTotal, req.TotalPayment, req.PartialAmount)));

    [HttpPatch("tax/{id}/assign")]
    public async Task<IActionResult> AssignTaxStaff(Guid id, [FromBody] AssignStaffRequest req)
    {
        var record = await _auditService.GetRecordByIdAsync<Domain.Entities.TaxAccountRecord>(id);
        if (record == null) return NotFound(ApiResponse<AuditRecordResponse>.Fail("Record not found."));
        
        // This is a bit specific to Tax, we could generalize it in AuditService if needed
        // For now, let's just use the UpdateRecordAsync or a specific method if we added it to IAuditService
        return Ok(ApiResponse<AuditRecordResponse>.Ok(await _auditService.UpdateRecordAsync<Domain.Entities.TaxAccountRecord>(id, new UpdateAuditRecordRequest { AssignedToId = req.AssignedToId })));
    }
