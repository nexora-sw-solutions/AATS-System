...
public abstract partial class TaxTableViewModelBase : ViewModelBase
{
    ...
    // --- Add Record State ---
    public virtual bool IsAddRecordFormVisible => true;
    public virtual bool IsAddRecordButtonVisible => false;
    ...
    partial void OnSelectedRecordChanged(TaxRecord? value)
    {
        if (value != null)
        {
            // Populate mock documents for preview when a record is selected
            PreviewDocuments.Clear();
            PreviewDocuments.Add($"Payment_Slip_{value.Code ?? "1"}.pdf");
        }
    }
    ...
    protected virtual void Details(TaxRecord record)
    {
        ...
        else
        {
            ...
            // Fetch client category if not present
            if (string.IsNullOrEmpty(record.ClientCategory) && !string.IsNullOrEmpty(record.ClientCode))
            {
                _ = Task.Run(async () =>
                {
                    try
                    {
                        var client = await DataService.Instance.GetClientByIdAsync(record.ClientCode);
                        if (client != null)
                        {
    ...
    protected void ApplyFilter()
    {
        ...
        var sorted = _filteredRecords.OrderBy(r => r.Code).ToList();
        _filteredRecords.Clear();
    ...
    {
        Console.WriteLine($"[DEBUG] Submit triggered for category: {PageTitle}");
        if (string.IsNullOrWhiteSpace(ClientId) && string.IsNullOrWhiteSpace(ClientName)) 
        {
            Console.WriteLine("[DEBUG] Submit aborted: ClientId and ClientName are both empty.");
            HasFormError = true;
        ...
        try
        {
            ...
            // Automatically resolve client details if client code is entered but Guid/Name is missing
            if (!_selectedClientGuid.HasValue && !string.IsNullOrWhiteSpace(ClientId))
            {
                var matchedClient = SharedClientsList.FirstOrDefault(c => string.Equals(c.ClientCode, ClientId, StringComparison.OrdinalIgnoreCase));
            ...
            var record = new TaxRecord
            {
                ...
                // New fields for TaxFiling alignment
                TaxType = MapPageTitleToTaxType(PageTitle),
                PeriodType = DurationUnit.EndsWith("s") ? DurationUnit.Substring(0, DurationUnit.Length - 1) : DurationUnit,
                PeriodNumber = Duration.ToString(),
                Process = "Pending",
                Notes = string.IsNullOrEmpty(EditAuditorNotes) ? "New record" : EditAuditorNotes
            };

            Console.WriteLine($"[DEBUG] Attempting to add tax record for client: {record.ClientName}");
            await DataService.Instance.AddTaxRecordAsync(PageTitle, record);
            Console.WriteLine("[DEBUG] Tax record submission successful.");
        ...
        {
            Console.WriteLine($"[DEBUG] Error submitting tax record: {ex.Message}");
            HasFormError = true;
            FormErrorMessage = $"Failed to save record: {ex.Message}";
        }
    }

    private string MapPageTitleToTaxType(string title)
    {
        if (title.Contains("VAT")) return "VAT";
        if (title.Contains("CIT")) return "CIT";
        if (title.Contains("IIT")) return "IIT";
        if (title.Contains("SSCL")) return "SSCL";
        if (title.Contains("WHT")) return "WHT";
        return "Others";
    }
