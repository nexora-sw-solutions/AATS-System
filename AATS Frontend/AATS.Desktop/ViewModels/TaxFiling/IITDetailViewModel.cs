...
{
    public partial class IITDetailViewModel : ViewModelBase
    {
        ...
        public string DisplayTaxpayerId => Record?.TIN ?? "N/A";
        ...
        [ObservableProperty] private string _editTaxpayerId = string.Empty;
        ...
        public IITDetailViewModel(TaxRecord record)
        {
            Record = record;
            if (record != null)
            {
        ...
        {
            try {
                RecordReportService.Instance.DownloadReportAsync(Record, "IIT", MainViewModel.Instance?.CurrentUser?.Username ?? "System");
            } catch (Exception ex) {
        ...
        {
            try {
                RecordReportService.Instance.PrintReportAsync(Record, "IIT", MainViewModel.Instance?.CurrentUser?.Username ?? "System");
            } catch (Exception ex) {
        ...
        private void EnterEditMode()
        {
            ...
            {
                MainViewModel.Instance.NavigateToIITEditRecord(Record);
            }
        }
        ...
        private void Submit()
        {
            ...
            OnPropertyChanged(nameof(DisplayTaxpayerId));
