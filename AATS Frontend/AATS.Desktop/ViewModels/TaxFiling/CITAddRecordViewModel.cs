...
{
    public partial class CITAddRecordViewModel : ViewModelBase
    {
        ...
        public CITAddRecordViewModel()
        {
            _ = LoadClientCodesAsync();
        }

        public CITAddRecordViewModel(TaxRecord record)
        {
            _ = LoadClientCodesAsync();
            ...
            DirectorId = record.DINNo ?? string.Empty;
            PaymentStatus = record.Status ?? "Pending";
            AuditorNotes = record.Notes ?? string.Empty;
            AdditionalInfo = "2024/2025"; // Hardcoded Assessment Year in display? Or let's see where it gets it from. Actually, wait. CITAddRecordView has AdditionalInfo bound to Assessment Year right now.
        ...
        public async Task SubmitAsync()
        {
            ...
            try
            {
                ...
                if (IsEdit && _originalRecord != null)
                {
                    ...
                    _originalRecord.DINNo = DirectorId;
                    _originalRecord.TIN = DirectorId;
                    ...
                    await DataService.Instance.UpdateTaxRecordAsync("Corporate Income Tax (CIT)", _originalRecord);
                }
                else
                {
                    ...
                    await DataService.Instance.AddTaxRecordAsync("Corporate Income Tax (CIT)", record);
                }
                GoBack?.Invoke();
            }
            catch (Exception ex)
