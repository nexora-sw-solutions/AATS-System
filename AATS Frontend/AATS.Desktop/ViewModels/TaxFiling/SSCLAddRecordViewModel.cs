...
{
    public partial class SSCLAddRecordViewModel : ViewModelBase
    {
        ...
        public SSCLAddRecordViewModel()
        {
            _ = LoadClientCodesAsync();
        }

        public SSCLAddRecordViewModel(TaxRecord record)
        {
            _ = LoadClientCodesAsync();
            ...
            SsclNo = record.TIN ?? string.Empty;
            PaymentStatus = record.Status ?? "Pending";
            AuditorNotes = record.Notes ?? string.Empty;
            AdditionalInfo = "2024/2025"; // Hardcoded Assessment Year in display? Or let's see where it gets it from. Actually, wait. SSCLAddRecordView has AdditionalInfo bound to Assessment Year right now.
        ...
        public async Task SubmitAsync()
        {
            ...
            try
            {
                ...
                if (IsEdit && _originalRecord != null)
                {
                    ...
                    await DataService.Instance.UpdateTaxRecordAsync("Social Security Contribution Levy (SSCL)", _originalRecord);
                }
                else
                {
                    ...
                    await DataService.Instance.AddTaxRecordAsync("Social Security Contribution Levy (SSCL)", record);
                }
                GoBack?.Invoke();
            }
            catch (Exception ex)
