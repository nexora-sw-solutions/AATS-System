...
{
    public partial class CITDetailViewModel : ViewModelBase
    {
        ...
        public string DisplayDirectorId => Record?.DINNo ?? "N/A";
        ...
        public CITDetailViewModel(TaxRecord record)
        {
            Record = record;
            if (record != null)
            {
        ...
        {
            try {
                RecordReportService.Instance.DownloadReportAsync(Record, "CIT", MainViewModel.Instance?.CurrentUser?.Username ?? "System");
            } catch (Exception ex) {
        ...
        {
            try {
                RecordReportService.Instance.PrintReportAsync(Record, "CIT", MainViewModel.Instance?.CurrentUser?.Username ?? "System");
            } catch (Exception ex) {
        ...
        private void EnterEditMode()
        {
            ...
            {
                MainViewModel.Instance.NavigateToCITEditRecord(Record);
            }
        }
