...
{
    public partial class IITAddRecordViewModel : ViewModelBase
    {
        ...
        [ObservableProperty] private string _TaxpayerId = string.Empty;
        ...
        public IITAddRecordViewModel()
        {
            _ = LoadClientCodesAsync();
        }

        public IITAddRecordViewModel(TaxRecord record)
        {
            _ = LoadClientCodesAsync();
            ...
            TaxpayerId = record.TIN ?? string.Empty;
            PaymentStatus = record.Status ?? "Pending";
            AuditorNotes = record.Notes ?? string.Empty;
            AdditionalInfo = "2024/2025"; // Hardcoded Assessment Year in display? Or let's see where it gets it from. Actually, wait. IITAddRecordView has AdditionalInfo bound to Assessment Year right now.
        ...
        public async Task SubmitAsync()
        {
            ...
            try
            {
                ...
                if (IsEdit && _originalRecord != null)
                {
                    ...
                    _originalRecord.TIN = TaxpayerId;
                    ...
                    await DataService.Instance.UpdateTaxRecordAsync("Individual Income Tax (IIT)", _originalRecord);
                }
                else
                {
                    ...
                    await DataService.Instance.AddTaxRecordAsync("Individual Income Tax (IIT)", record);
                }
                GoBack?.Invoke();
            }
            catch (Exception ex)
