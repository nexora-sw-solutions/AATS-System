...
{
    public partial class VATDetailViewModel : ViewModelBase
    {
        ...
        public string DisplayVatNo => Record?.TIN ?? "N/A";
        ...
        [ObservableProperty] private string _editVatNo = string.Empty;
        ...
        public VATDetailViewModel(TaxRecord record)
        {
            Record = record;
            if (record != null)
            {
        ...
        {
            try {
                RecordReportService.Instance.DownloadReportAsync(Record, "VAT", MainViewModel.Instance?.CurrentUser?.Username ?? "System");
            } catch (Exception ex) {
        ...
        {
            try {
                RecordReportService.Instance.PrintReportAsync(Record, "VAT", MainViewModel.Instance?.CurrentUser?.Username ?? "System");
            } catch (Exception ex) {
        ...
        private void EnterEditMode()
        {
            ...
            {
                MainViewModel.Instance.NavigateToVATEditRecord(Record);
            }
        }
        ...
        private void Submit()
        {
            ...
            OnPropertyChanged(nameof(DisplayVatNo));
