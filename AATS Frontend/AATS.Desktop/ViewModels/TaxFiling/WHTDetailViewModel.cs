...
{
    public partial class WHTDetailViewModel : ViewModelBase
    {
        ...
        public string DisplayWhtNo => Record?.TIN ?? "N/A";
        ...
        [ObservableProperty] private string _editWhtNo = string.Empty;
        ...
        public WHTDetailViewModel(TaxRecord record)
        {
            Record = record;
            if (record != null)
            {
        ...
        {
            try {
                RecordReportService.Instance.DownloadReportAsync(Record, "WHT", MainViewModel.Instance?.CurrentUser?.Username ?? "System");
            } catch (Exception ex) {
        ...
        {
            try {
                RecordReportService.Instance.PrintReportAsync(Record, "WHT", MainViewModel.Instance?.CurrentUser?.Username ?? "System");
            } catch (Exception ex) {
        ...
        private void EnterEditMode()
        {
            ...
            {
                MainViewModel.Instance.NavigateToWHTEditRecord(Record);
            }
        }
        ...
        private void Submit()
        {
            ...
            OnPropertyChanged(nameof(DisplayWhtNo));
