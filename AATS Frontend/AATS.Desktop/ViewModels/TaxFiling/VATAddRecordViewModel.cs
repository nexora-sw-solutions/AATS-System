...
{
    public partial class VATAddRecordViewModel : ViewModelBase
    {
        ...
        [ObservableProperty] private string _VatNo = string.Empty;
        ...
        public VATAddRecordViewModel()
        {
            _ = LoadClientCodesAsync();
        }

        public VATAddRecordViewModel(TaxRecord record)
        {
            _ = LoadClientCodesAsync();
            ...
            VatNo = record.TIN ?? string.Empty;
            PaymentStatus = record.Status ?? "Pending";
            AuditorNotes = record.Notes ?? string.Empty;
            AdditionalInfo = "2024/2025"; // Hardcoded Assessment Year in display? Or let's see where it gets it from. Actually, wait. VATAddRecordView has AdditionalInfo bound to Assessment Year right now.
        ...
        public async Task SubmitAsync()
        {
            ...
            try
            {
                ...
                if (IsEdit && _originalRecord != null)
                {
                    ...
                    await DataService.Instance.UpdateTaxRecordAsync("Value Added Tax (VAT)", _originalRecord);
                }
                else
                {
                    ...
                    await DataService.Instance.AddTaxRecordAsync("Value Added Tax (VAT)", record);
                }
                GoBack?.Invoke();
            }
            catch (Exception ex)
