...
public partial class StaffDetailViewModel : DetailViewModelBase
{
    ...
    [ObservableProperty] private bool _isAddTransactionVisible;
    [ObservableProperty] private DateTime? _newTransactionDate = DateTime.Now;
    [ObservableProperty] private string _newTransactionDescription = "";
    [ObservableProperty] private string _newTransactionAmountText = "";
    ...
    private async Task SaveNewTransaction()
    {
        ...
        ConfirmDialogMessage = "Are you sure you want to add this transaction record?";
        ConfirmActionDelegate = async () =>
        {
            var newHistory = new StaffHistory
            {
                Date = NewTransactionDate ?? DateTime.Now,
                Description = NewTransactionDescription,
                Amount = amount
            };

            _staffMember.History ??= new List<StaffHistory>();
            _staffMember.History.Insert(0, newHistory);
            History = new ObservableCollection<StaffHistory>(_staffMember.History);
    ...
    private void CancelAddTransaction()
    {
        if (!string.IsNullOrEmpty(NewTransactionDescription) || !string.IsNullOrEmpty(NewTransactionAmountText))
        {
            ...
            ConfirmDialogMessage = "You have unsaved changes. Are you sure you want to cancel?";
            ConfirmActionDelegate = () =>
            {
                IsAddTransactionVisible = false;
