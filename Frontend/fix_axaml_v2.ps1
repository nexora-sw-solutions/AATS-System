$oldSnippet = @'
                                                                                <TextBox Classes="form-input" Watermark="Enter Client ID (e.g. CL-00001)" Text="{Binding Id}" Name="ClientIdTextBox"/>
                                                            ...
                                                            <ItemsControl.ItemTemplate>
                                                                <DataTemplate>
                                                                    <Button Background="Transparent" 
                                                                            ...
                                                                            Cursor="Hand"
                                                                            Command="{Binding $parent[UserControl].DataContext.SelectClientRecordCommand}" 
...
# Also handle the variant from Audit files
$auditOldSnippet = @'
                                            <TextBox Classes="form-input" Watermark="Enter Client ID (e.g. CL-00001)" Text="{Binding ClientId}" Name="ClientIdTextBox"/>
...
foreach ($dir in $directories) {
    $files = Get-ChildItem -Path $dir -Filter "Add*View.axaml"
    foreach ($f in $files) {
        ...
        if ($content.Contains($auditOldSnippet)) {
             $content = $content.Replace($auditOldSnippet, $newSnippet)
             $replaced = $true
        }
